...
@php
    $icons = [
        ...
        'danger' => '<svg class="alert-icon" fill="none" stroke="currentColor" viewBox="0 0 24 24"><path stroke-linecap="round" stroke-linejoin="round" stroke-width="2" d="M12 8v4m0 4h.01M21 12a9 9 0 11-18 0 9 9 0 0118 0z"/></svg>',
...
<div x-data="{ show: true }" x-show="show" x-transition {{ $attributes->merge(['class' => "alert alert-{$type}"]) }}>
    {!! $icons[$type] !!}
    ...
    @if($dismissible)
        <button type="button" @click="show = false" class="ml-auto text-current opacity-50 hover:opacity-100">
