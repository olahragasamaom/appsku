...
@section('breadcrumb')
    ...
    <span class="text-secondary-900 font-medium">Tambah Paket</span>
@endsection

@section('header')
    <div class="flex items-center justify-between gap-4">
        <div>
            <h1 class="text-2xl font-bold text-secondary-900">Tambah Paket Member</h1>
            <p class="text-secondary-500 mt-1">Buat paket langganan baru untuk peserta</p>
        </div>
                ...
                <div class="grid grid-cols-1 md:grid-cols-2 gap-6">
                    ...
                    <div class="md:col-span-2">
                        ...
                        <textarea name="deskripsi" id="deskripsi" rows="3"
                                  class="input w-full @error('deskripsi') border-danger-500 @enderror">{{ old('deskripsi') }}</textarea>
                        @error('deskripsi')
                            <p class="mt-1 text-sm text-danger-600">{{ $message }}</p>
                        @enderror
                    </div>
                ...
                <div class="mt-6 border-t border-secondary-100 pt-6">
                    ...
                    <div class="flex flex-wrap gap-6">
                        <label class="flex items-center">
                            <input type="checkbox" name="video_pembahasan" value="1" {{ old('video_pembahasan') ? 'checked' : '' }} class="rounded border-secondary-300 text-primary-600 shadow-sm focus:ring-primary-500">
                            <span class="ml-2 text-sm text-secondary-600">Video Pembahasan</span>
                        </label>
                        <label class="flex items-center">
                            <input type="checkbox" name="analitik" value="1" {{ old('analitik') ? 'checked' : '' }} class="rounded border-secondary-300 text-primary-600 shadow-sm focus:ring-primary-500">
                            <span class="ml-2 text-sm text-secondary-600">Analitik Lanjutan</span>
                        </label>
                        <label class="flex items-center">
                            <input type="checkbox" name="sertifikat" value="1" {{ old('sertifikat') ? 'checked' : '' }} class="rounded border-secondary-300 text-primary-600 shadow-sm focus:ring-primary-500">
                            <span class="ml-2 text-sm text-secondary-600">Sertifikat Kelulusan</span>
                        </label>
                        <label class="flex items-center">
                            <input type="checkbox" name="is_active" value="1" {{ old('is_active', '1') ? 'checked' : '' }} class="rounded border-secondary-300 text-primary-600 shadow-sm focus:ring-primary-500">
